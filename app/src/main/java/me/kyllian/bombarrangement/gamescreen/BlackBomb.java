...
public class BlackBomb extends Bomb {
    ...
    public BlackBomb(Bitmap res, int x, int y, int numFrames) {
        super.x = x;
        super.y = y;
        ...
        for (int i = 0; i < image.length; i++) {
            image[i] = Bitmap.createBitmap(spritesheet, 0, i * 105, res.getWidth(), 105);
        }
            ...
            y += y > goaly ? -5 : 5;
                if (!Panel.isLegalMovement(this)) {
                    if (!isSorted()) {
                        ...
                        this.setGoal(ThreadLocalRandom.current().nextInt(612, 1080), ThreadLocalRandom.current().nextInt(1069, 1597));
                        update();
                        return;
                    }
                }
